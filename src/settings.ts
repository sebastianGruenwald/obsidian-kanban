	...
	private displayBoardSettings(containerEl: HTMLElement, board: BoardConfig): void {
		...
		new Setting(contentDiv)
			...
			.addDropdown(dropdown => dropdown
				...
				.onChange(async (value) => {
					...
					await this.plugin.saveSettings();
					this.plugin.refreshAllViews();
					// Re-render settings to update font dropdown
					this.display();
				}));

		// Font selection for current theme
		const currentTheme = board.theme || 'sticky-notes';
		const themeFonts = board.themeFonts || { 'default': '', 'sticky-notes': '' };
		...
		new Setting(contentDiv)
			...
			.addDropdown(dropdown => {
				...
				dropdown.addOption('var(--font-interface)', '⭐ Interface (System UI)');
				dropdown.addOption('var(--font-text)', '⭐ Text (Reading)');
				dropdown.addOption('var(--font-monospace)', '⭐ Monospace (Code)');
				
				// Handwriting fonts (good for sticky notes)
				dropdown.addOption('__separator2__', '── Handwriting ──');
				dropdown.addOption('Caveat, cursive', 'Caveat (Default Sticky)');
				dropdown.addOption('Comic Sans MS, cursive', 'Comic Sans MS');
				dropdown.addOption('Brush Script MT, cursive', 'Brush Script');
				dropdown.addOption('Segoe Script, cursive', 'Segoe Script');
				
				// Serif fonts
				dropdown.addOption('__separator3__', '── Serif ──');
				dropdown.addOption('Georgia, serif', 'Georgia');
				dropdown.addOption('Times New Roman, serif', 'Times New Roman');
				dropdown.addOption('Palatino Linotype, serif', 'Palatino');
				dropdown.addOption('Book Antiqua, serif', 'Book Antiqua');
				dropdown.addOption('Garamond, serif', 'Garamond');
				
				// Sans-serif fonts
				dropdown.addOption('__separator4__', '── Sans-Serif ──');
				dropdown.addOption('Arial, sans-serif', 'Arial');
				dropdown.addOption('Helvetica, sans-serif', 'Helvetica');
				dropdown.addOption('Verdana, sans-serif', 'Verdana');
				dropdown.addOption('Trebuchet MS, sans-serif', 'Trebuchet MS');
				dropdown.addOption('Tahoma, sans-serif', 'Tahoma');
				dropdown.addOption('Segoe UI, sans-serif', 'Segoe UI');
				
				// Monospace fonts
				dropdown.addOption('__separator5__', '── Monospace ──');
				dropdown.addOption('Consolas, monospace', 'Consolas');
				dropdown.addOption('Courier New, monospace', 'Courier New');
				dropdown.addOption('Fira Code, monospace', 'Fira Code');
				dropdown.addOption('Monaco, monospace', 'Monaco');
				
				dropdown.setValue(currentFont);
				dropdown.onChange(async (value) => {
					// Skip separator options
					if (value.startsWith('__separator')) {
						dropdown.setValue(currentFont);
						return;
					}
					const newThemeFonts = { ...themeFonts, [currentTheme]: value };
					this.plugin.boardManager.updateBoard(board.id, { themeFonts: newThemeFonts });
					await this.plugin.saveSettings();
					this.plugin.refreshAllViews();
				});
				...
				setTimeout(() => {
					...
					Array.from(selectEl.options).forEach(opt => {
						if (opt.value.startsWith('__separator')) {
							opt.disabled = true;
							opt.style.fontWeight = 'bold';
							opt.style.color = 'var(--text-muted)';
							opt.style.backgroundColor = 'var(--background-secondary)';
						}
					});
				}, 0);
			});
